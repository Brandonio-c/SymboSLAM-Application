...
#include <cstdlib>
#include <cstring>
#include "../include/Random_Drive_V2.h"
#include "eyebot++.h"
...
void scan_area(BYTE img[QVGA_SIZE], DarkHelp::NN *nn, local_map *local_map, int scan[360], int SAFE, int scaling, int scan_loc){
    int dis_to_feat = scan[scan_loc];;
    CAMGet(img);    // edge agent camera demo
    LCDImage(img);  // only
        ...
        try
        {
            ...
            while(dis > SAFE){
                ...
                else
                {
                    ...
                    int phi = getPosition(true, local_map)->phi;
                    VWTurn((360 - phi), 90); VWWait(); // turn [-90..+90]
                    // take a lidar scan of the current area
                    LIDARGet(scan);
                    //make x turns to capture a full 360 of this current area through camera
                    int num_turns = math.ceil(360/FOV);
                    int deg_turn = math.ceil(360/num_turns);
                    vector<detected_feature> detected_feature_list;
                    for(int idx = 0; idx < num_turns; idx++){
                        scan_area(img, &nn, local_map, scan, SAFE, scaling, idx);
